...
>(({ className, ...props }, ref) => {
  return (
    <button
      ...
      {...props}
    >
      <span className="absolute -bottom-px left-1/2 h-1/2 w-3/4 -translate-x-1/2 bg-[radial-gradient(closest-side,rgba(255,255,255,0.25),transparent)] opacity-100"></span>
      {props.children}
    </button>
  );
});
